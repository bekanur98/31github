...
//solution with map
function sortTheStudents(score: number[][], k: number): number[][] {
    const queueOfColumns = new Map<number, number>();
    let i = 0;
    for (const exams of score) {
        queueOfColumns.set(i, exams[k]);
        i++;
    }

    const sortedScores = [];
    const mapSort = new Map([...queueOfColumns.entries()].sort((a, b) => b[1] - a[1]));
    for (let [key, value] of mapSort) {
        sortedScores.push(score[key])
    }
